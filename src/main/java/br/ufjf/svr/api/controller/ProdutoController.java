...
@CrossOrigin
@Tag(name = "Produtos", description = "Operações relacionadas a produtos. Acesso público (não requer autenticação) para todos os endpoints.")
public class ProdutoController {
    ...
    public ResponseEntity get(@Parameter(description = "ID do produto a ser buscado") @PathVariable("id") Long id) {
    ...
    public ResponseEntity atualizar(@Parameter(description = "ID do produto a ser atualizado") @PathVariable("id") Long id, @RequestBody ProdutoDTO dto) {
    ...
    public ResponseEntity excluir(@Parameter(description = "ID do produto a ser excluído") @PathVariable("id") Long id) {
        Optional<Produto> produto = service.getProdutoById(id);
        if (!produto.isPresent()) {
            return new ResponseEntity("Produto não encontrado", HttpStatus.NOT_FOUND);
        }
